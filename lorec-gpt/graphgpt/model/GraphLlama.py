...
def transfer_param_tograph(clip_graph, gnn):
    ...
    gnn.load_state_dict(gnn_state_dict, assign=True)
    return gnn
...
class GraphLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if hasattr(config, "graph_tower"):
            ...
            if config.graph_tower == 'MPNN': 
                ...
                self.graph_tower = graph_transformer(args)
                self.graph_tower = transfer_param_tograph(clip_graph, self.graph_tower)
            elif config.graph_tower == "clip_gt_cora": 
                clip_graph, args= load_model_pretrained(CLIP, config.pretrain_graph_model_path) 
                self.graph_tower = graph_transformer(args)
                self.graph_tower = transfer_param_tograph(clip_graph, self.graph_tower)
    ...
    def initialize_graph_modules(self, graph_tower, graph_select_layer,
        ...
        if not hasattr(self, 'graph_tower'):
            ...
            # graph_tower = MPNN(in_channels = self.config.graph_hidden_size, hidden_channels = self.config.graph_hidden_size * 2, out_channels = self.config.graph_hidden_size, dropout = 0.1, num_layers = 2)
                ...
                graph_tower = graph_transformer(args)
                graph_tower = transfer_param_tograph(clip_graph, graph_tower)
            elif self.config.graph_tower == "clip_gt_cora":
                clip_graph, args= load_model_pretrained(CLIP, self.config.pretrain_graph_model_path) 
                graph_tower = graph_transformer(args)
                graph_tower = transfer_param_tograph(clip_graph, graph_tower)
        else:
    ...
    def forward(
        ...
        # create
        # print('use original text embedding or not:', orig_embeds_params) # None
        ...
        if graph_tower is not None and (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
            ...
            with torch.no_grad():
                if type(graph_data) is list:
                    ...
                    graph_node_features = []
                    # Handle mixed-type lists (Data objects and dicts)
                    for g in graph_data:
                    ...
                    #         graph_node_features.append(node_forward_out_2)
                            ...
                            if 'graph_1' in g and 'graph_2' in g:
                                node_forward_out_1 = graph_tower(g['graph_1'])
                                node_forward_out_2 = graph_tower(g['graph_2'])
                                graph_node_features.append(node_forward_out_1)
                                graph_node_features.append(node_forward_out_2)
                            else:
                                # Unknown dict payload (e.g., placeholder for no-graph sample): skip
                                continue
                        else:
            ...
            if type(graph_data) is list:
                ...
                raise ValueError(f'graph_node_reps is expected to be a list but got {type(graph_data)}')
            # Use graph_projector's weight dtype to ensure compatibility
            projector_dtype = self.graph_projector.weight.dtype
            dummy_graph_features = torch.zeros(256, 128, device=inputs_embeds.device, dtype=projector_dtype)
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                # Processing samples of data without graphs
                if (cur_input_ids == graph_tower.config.graph_patch_token).sum() == 0:
                    ...
                    # Do not advance cur_graph_idx since no graph features are consumed
                    continue
                # Processing samples of data with graphs
                if graph_tower.config.use_graph_start_end:
                    ...
                    for graph_start_token_pos in graph_start_tokens:
                        ...
                        text_before = cur_input_embeds[:graph_start_token_pos+1]
                        text_after = cur_input_embeds[graph_start_token_pos + num_patches + 1:]
                        

                        # Replace graph tokens with real graph node features
                        if orig_embeds_params is not None:
                            # create
                    ...
                    text_before = cur_input_embeds[:mask_index_start]
                    text_after = cur_input_embeds[mask_index_start+num_patches:]
            ...
            if cur_graph_idx != len(graph_node_features):
                try:
                    print(f"[Warn][GraphLlamaModel] Consumed graphs {cur_graph_idx} != provided graphs {len(graph_node_features)}. Proceeding without strict assert.")
                except Exception:
                    pass
...
class GraphLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config, *args, **kwargs):
        super(LlamaForCausalLM, self).__init__(config, *args, **kwargs)
    ...
    def forward(
        ...
        # edge_index_reps: Optional[torch.FloatTensor] = None,
        graph_data: Optional[Data] = None,
        # GCD (Graph Contrastive Decoding) parameters
        graph_data_cg: Optional[Data] = None,
        graph_data_cd: Optional[Data] = None,
        cd_alpha: Optional[float] = None,
        cg_beta: Optional[float] = None,
        return_dict: Optional[bool] = None,
    ...
    def prepare_inputs_for_generation(
        ...
        # choose attention_mask: prefer explicit arg, else fallback to kwargs
        attn_mask = attention_mask if attention_mask is not None else kwargs.get("attention_mask", None)
        model_inputs.update(
            {
                "past_key_values": past_key_values,
                "use_cache": kwargs.get("use_cache"),
                "attention_mask": attn_mask,
                "graph_data": (kwargs.get("graph_data", None) if (kwargs.get("graph_data", None) is None or isinstance(kwargs.get("graph_data", None), list)) else [kwargs.get("graph_data", None)]),
                # "edge_index_reps": kwargs.get("edge_index_reps", None),
            }
        )
        
        # Handle GCD (Graph Contrastive Decoding) parameters
        # These parameters are used by custom generation methods but should not be passed to the base generate()
        # They will be extracted and handled separately in the generation loop
        gcd_params = {}
        for param_name in ['graph_data_cg', 'graph_data_cd', 'cd_alpha', 'cg_beta']:
            if param_name in kwargs:
                gcd_params[param_name] = kwargs[param_name]
        
        # Store GCD parameters in model_inputs if they exist
        if gcd_params:
            model_inputs.update(gcd_params)
        
        return model_inputs

    def _validate_model_kwargs(self, model_kwargs: dict):
        """
        Override the validation to allow GCD-specific parameters.
        
        This method allows the following custom parameters that are not part of the base model:
        - graph_data: Graph data for the model
        - graph_data_cg: Augmented graph data for contrastive learning
        - graph_data_cd: Text-only graph data for contrastive learning
        - cd_alpha: Hyperparameter for text-only contrast
        - cg_beta: Hyperparameter for augmented graph contrast
        """
        # List of parameters that are allowed but not part of the base model signature
        allowed_custom_params = {
            'graph_data', 'graph_data_cg', 'graph_data_cd', 'cd_alpha', 'cg_beta'
        }
        
        # Create a copy of model_kwargs to avoid modifying the original
        model_kwargs_copy = model_kwargs.copy()
        
        # Remove allowed custom parameters before calling parent validation
        for param in allowed_custom_params:
            model_kwargs_copy.pop(param, None)
        
        # Call parent validation with the filtered kwargs
        super()._validate_model_kwargs(model_kwargs_copy)
