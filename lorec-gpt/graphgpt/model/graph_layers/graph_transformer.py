...
class graph_transformer(nn.Module):
    ...
    def forward(self, g):
        ...
        # Handle None input (e.g., when graph_data is None in text-only mode)
        if g is None:
            raise ValueError("graph_transformer.forward() received None as input. "
                           "This typically happens when graph_data is None. "
                           "Ensure graph data is provided or disable graph processing.")
        
        device = self.parameters().__next__().device
        g = g.to(device)
        
        x = getattr(g, 'graph_node', None)
        if x is None:
            x = getattr(g, 'x', None)
        if x is None:
            raise AttributeError("Graph data missing node features: expected 'graph_node' or 'x'.")
        ...
        x = x.to(device=self.W_P.weight.device, dtype=self.W_P.weight.dtype)
        z = self.W_P(x)
        if self.args.if_pos:
            pos = self.W_pos.to(device=z.device, dtype=z.dtype)
            embeds = self.dropout(z + pos)
        else:
            embeds = self.dropout(z)
        for gt in self.gtLayers:
            embeds = gt(g, embeds) # bs * num_patch * n * d_model

        embeds = embeds.to(device=self.inverW_P.weight.device, dtype=self.inverW_P.weight.dtype)
